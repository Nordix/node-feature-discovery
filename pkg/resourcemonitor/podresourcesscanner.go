...
func (resMon *PodResourcesScanner) Scan() (ScanResponse, error) {
	...
	if resMon.podFingerprint && len(respPodResources) > 0 {
		...
		if err != nil {
			...
			klog.V(2).InfoS("podFingerprint calculated", "status", status.Repr())
	...
	for _, podResource := range respPodResources {
		klog.V(2).InfoS("scanning pod", "podName", podResource.GetName())
...
func hasDevice(podResource *podresourcesapi.PodResources) bool {
	...
	klog.V(2).InfoS("pod doesn't have devices", "podName", podResource.GetName())
	return false
}
